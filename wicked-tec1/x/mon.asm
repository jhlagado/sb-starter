...
0066   DB 00                        IN      a,(0x00)        ;input a from keyboard             a = PORT(0)
0068   E6 1F                        AND     0x1f            ;mask lower 5 bits                 a = a & $1F
006A   ED 47                        LD      i,a             ;move to i register                i = a
006C   C9                           RET                     ;BUG: should be RETN               RET
006D                                                        ;b   ranches here after WRITEDISP
006D   31 D0 0F     WRITEDISP2:     LD      sp,STARTSTACK                                      sp = STARTSTACK
0070   CD 31 01                     CALL    GETKEY          ;(blocking) get key                GETKEY()
0073   CD 8E 01                     CALL    BEEP            ;beep                              BEEP()
0076   3A F9 0F                     LD      a,(MODE)        ;a = addr/data                     a = (MODE)
0079   FE 00                        CP      0x00            ;z = (MODE == ADDR_MODE)           a == 0
007B   C2 96 00                     JP      nz,WRITEDISP3                                      IF !z
007E                                                        ;i   s ADDR_MODE
007E   ED 57                        LD      a,i             ;a = key                                a = i
0080   FE 10                        CP      0x10                                                    a == $10
0082   DA 88 00                     JP      c,ADDRKEY       ;key is numeric                         IF lt
0085   C3 E3 00                     JP      DATADISP        ;change to DATA_MODE                        goto DATADISP
0088   21 F7 0F     ADDRKEY:        LD      hl,ADDRESS                                              hl = ADDRESS
008B   CD 6F 01                     CALL    GETADDRKEY      ;getkey and clear 16 bits if !KEYFLAG   a = GETADDRKEY()
008E   ED 6F                        RLD                     ;rotate left nibble (hl) <- a           (hl) = a & $F0 >> 4
0090   23                           INC     hl              ;point to upper byte                    hl++
0091   ED 6F                        RLD                     ;rotate left nibble (hl) <- a           (hl) = a & $F0 >> 4
0093   C3 DA 00                     JP      ADDRDISP        ;change to ADDR_MODE,                   goto ADDRDISP
0096   ED 57        WRITEDISP3:     LD      a,i             ;is DATA_MODE                       a = i
0098   FE 10                        CP      0x10                                                a == $10
009A   DA B7 00                     JP      c,DATAKEY       ;key is numeric                     if lt
009D   AF                           XOR     a               ;is function key                        a = 0
009E   32 FA 0F                     LD      (KEYFLAG),a     ;(KEYFLAG)=false init key after mode    (KEYFLAG) = a
00A1   ED 57                        LD      a,i             ;a = key value                          a = i
00A3   FE 13                        CP      0x13            ;(AD) MODE key                          z = a == $13
00A5   CA DA 00                     JP      z,ADDRDISP                                              if z goto ADDRDISP
00A8   FE 12                        CP      0x12            ;(GO) KEY                               z = a == $12
00AA   CA C2 00                     JP      z,GOADDR                                                if z goto GOADDR
00AD   FE 11                        CP      0x11            ;(-) KEY                                z = a == $11
00AF   CA C6 00                     JP      z,DECADDR                                               if z goto DECADDR
00B2   FE 10                        CP      0x10            ;(+) KEY                                z = z == $10
00B4   CA D0 00                     JP      z,INCADDR                                               if z goto INCADDR
00B7   2A F7 0F     DATAKEY:        LD      hl,(ADDRESS)                                          hl = (ADDR)
00BA   CD 7B 01                     CALL    GETDATAKEY      ;getkey and clear 16 bits if !KEYFLAG a = GETDATAKEY()
00BD   ED 6F                        RLD                     ;rotate left nibble (hl) <- a         (hl) = a & $F0 >> 4
00BF   C3 E3 00                     JP      DATADISP                                              goto DATADISP
00C2   2A F7 0F     GOADDR:         LD      hl,(ADDRESS)                                          hl = (ADDRESS)
00C5   E9                           JP      (hl)            ;jump (ADDRESS)                       jp (hl)
00C6   2A F7 0F     DECADDR:        LD      hl,(ADDRESS)    ;(ADDRESS)--                          hl = (ADDRESS)
00C9   2B                           DEC     hl                                                    hl--
00CA   22 F7 0F                     LD      (ADDRESS),hl                                          (ADDRESS) = hl
